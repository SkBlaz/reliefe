# This is the main code for ReliefE algorithm.
# By Skrlj and Petkovic 2020
...
@jit(nopython=True)
def _compiled_classification_update_weights(samples, num_iter, data, pointers, indices, priors, k, pairwise_distances,
                                            class_members, examples_to_class, class_first_indices, nrow_all, ncol_all,
    ...
    weights_final = np.zeros((len(num_iter), number_of_columns), dtype=np.float64)
    ...
    for i_sample, sample in enumerate(samples):
        ...
        for c, number_of_samples in enumerate(class_first_indices[:-1]):
            ...
            for i_neighbor, neighbour in enumerate(members):
                ...
                internal_distance = _numba_distance(ith_sample_row, neigh_row, pairwise_distances)
                distances[i_neighbor] = internal_distance
            top_neighbour_indices = np.argsort(distances)

            if determine_k_automatically:
                sorted_distances = distances[top_neighbour_indices] #np.sort(distances)
            ...
            nearest_neighbours = np.zeros((len(top_neighbor_members), ndim_via_pointer))
            ...
            for j in range(number_of_columns):
                ...
                if use_average_neighbour:
                    diff = np.abs(sample_row[j] - np.mean(nearest_neighbours[:, j]))
                    
                else:
                    diff = np.mean(np.abs(sample_row[j] - nearest_neighbours[:, j]))
...
@jit(nopython=True)
def _compiled_multi_label_classification_update_weights(samples, num_iter,
                                                        data, pointers, indices, data_y, pointers_y, indices_y,
                                                        k, pairwise_distances, mlc_distance, nrow, ncol):
    ...
    for i_sample, sample in enumerate(samples):
        ...
        nearest_neighbours = np.zeros((len(top_neighbor_indices), ndim_via_pointer))
        ...
        for i in range(k):
            ...
            tdist = _numba_distance_target(y_sample_row, ith_y_row, mlc_distance)
        ...
        for j in range(number_of_columns):
            # Difference between near hits/misses for the j-th feature
            descriptive_diffs = np.abs(sample_row[j] - nearest_neighbours[:, j])
            d_diff = np.mean(descriptive_diffs)
            t_d_diff = np.mean(target_diffs * descriptive_diffs)
            weights[j] += t_d_diff / t_diff - (d_diff - t_diff) / (1.0 - t_diff)
...
class ReliefE:
    def __init__(self, num_iter: Union[float, int, List[Union[float, int]]] = 1.0, k=8, normalize_descriptive=True,
                 embedding_based_distances=False,
                 num_threads="all", verbose=False,
                 mlc_distance="f1",
    ...
    def determine_latent_dim(self, xs, true_dim=None, empirical_left_continuous=True, metric = "euclidean"):
        """
        ...
        # visualize dists -> uncomment this
        
        # ks = []        
        # for j in range(dists.shape[0]):
        #     diffs = np.diff(dists[j,:])
        #     fdiffs = diffs[1:-1]
        #     fdiffs2 = diffs[2:]
        #     ratios = np.divide(fdiffs2, fdiffs)
        ...
        if empirical_left_continuous:
            ...
            ok_indices = list(range(max_position)) + list(range(max_position + 1, xs.shape[0]))
            mu = mu[ok_indices]
        ...
        try:
          ...
          latent_dim, _, _, _ = np.linalg.lstsq(mu_first, mu_second, rcond=None)
        ...
        if self.verbose:
          logging.info("Estimated latent dimension is: {}".format(latent_dim))
            
        # if self.verbose:
        #     logging.info("Latent dimension established to be : {}".format(latent_dim))
        #     plt.plot(mu_first.reshape(-1), mu_second, '.', color = "black")
        #     plt.title("Latent dimension: {}".format(latent_dim))
        #     plt.plot([0, max(mu_first)], [0, -max(mu_first) * latent_dim], "r--",
        #              label="Computed dim {}".format(latent_dim))
        #     if true_dim is not None:
        #         plt.plot([0, max(mu_first)], [0, -max(mu_first) * true_dim], "g--",label="True dim {}".format(true_dim))
        #     plt.xlabel("$\log{(\mu)}$")
        #     plt.ylabel("$\log{(1 - f_{emp})}$")
        #     plt.legend()
        #     plt.show()
    ...
    def _check_parameters(self, n_examples, class_counts):
        ...
        for i, iterations in enumerate(self.num_iter):
            ...
            if self.num_iter[i] != iterations:
                logging.info(
                    message.format("iterations", iterations, min_iter, "number samples", n_examples, self.num_iter[i])
                )
        self.num_iter.sort()
        ...
        if self.k < 1:
            raise ValueError("All classes should have size at least 2! (class sizes: {})".format(class_counts))
          
        if k != self.k:
            logging.info(message.format("neighbours", k, min_k, "smallest class size", min_class, self.k))
            
        # mlc distance
        if self.mlc_distance not in MLCDistances.DISTANCES:
            raise ValueError("MLC distance {} should be en element of {}".format(self.mlc_distance,
                                                                                 MLCDistances.DISTANCES))
        # average neighbour
        if self.task_type not in [TaskTypes.CLASSIFICATION, TaskTypes.REGRESSION] and self.use_average_neighbour:
            self.use_average_neighbour = False
            logging.info("Cannot use average neighbour for {}. Setting the parameter to False.".format(self.task_type))
    ...
    def compute_members(y):
        ...
        class_first_indices = np.cumsum(class_first_indices)  # contains an additional element at the end
        return np.concatenate(members), np.array(example_to_class), class_first_indices
    ...
    def fit(self, x, y):
        ...
        if self.verbose:
            self.send_message("Dataset shape X: {} Y: {}".format(x.shape, y.shape))

        # omega_ram = max(round(((x.shape[0] * x.shape[1])*8)/10**9,2),round(((x.shape[0] * x.shape[0])*8)/10**9,2))

        # if omega_ram > self.max_ram:

        #   ## subsample
        #   self.send_message("Potential spatial overhead detected. Required RAM if densified: {}GB".format(omega_ram))
          
        #   ## subsample if too many instances
        #   subsamples = int((self.max_ram / (omega_ram)) * x.shape[0])
        #   omega_ram = max(round(((subsamples * x.shape[1])*8)/10**9,2),round(((subsamples * subsamples)*8)/10**9,2))

        subsamples = min(self.samples,x.shape[0])

        if self.samples < x.shape[0]:
            self.send_message("Subsampling to {} instances.".format(subsamples))
        ...
        sparsity_var = len(x_sampled.nonzero()[0])/(x_sampled.shape[1] * x_sampled.shape[0])
        ...
        if self.task_type == TaskTypes.CLASSIFICATION:
            ...
            class_counts = np.array([n_examples])  # needed so that resetting parameters works
        ...
        if self.embedding_based_distances:
          ...
          # Project to low dim
          self.send_message("Estimating embedding from {}.".format(x_sampled.shape))
          latent_dim = min(self.determine_latent_dim(x_sampled), 8)
          reducer = umap.UMAP(n_components=latent_dim, n_neighbors = self.k, low_memory=True, init = "spectral")
          ...
          self.send_message("Embedding obtained.")
            ...
            if self.normalize:
                factors = (np.max(x_embedded, axis=0) - np.min(x_embedded, axis=0)).todense()
        ...
        samples = np.random.RandomState(seed=42).permutation(n_examples)[:max(self.num_iter)]
        if self.embedding_based_distances:
            pairwise_distances = "cosine"
        else:
            pairwise_distances = "cityblock"  # cdist(x_embedded[samples], x_embedded, metric="cityblock")

        self.send_message("Using {} distance for the Relief iteration.".format(pairwise_distances))

        # Run the Numba kernel
        data, pointers, indices = x_embedded.data, x_embedded.indptr, x_embedded.indices
        if self.task_type == TaskTypes.CLASSIFICATION:
            weights = _compiled_classification_update_weights(samples,
                                                              self.num_iter,
                                                              data,
                                                              pointers,
                                                              indices,
                                                              class_priors,
                                                              self.k, pairwise_distances,
                                                              class_members,
                                                              examples_to_class,
                                                              class_first_indices,
                                                              nrow_raw,
                                                              ncol_raw,
                                                              self.determine_k_automatically,
                                                              self.use_average_neighbour)
        elif self.task_type == TaskTypes.MLC:
            ...
            if self.mlc_distance == "cosine":
                ...
                reducer = umap.UMAP(n_components=latent_dim, n_neighbors = self.k, low_memory=True, init = "spectral")
                y = sparse.csr_matrix(reducer.fit(y[indices_sample]).transform(y))

            elif self.mlc_distance == "hyperbolic":
                reducer = umap.UMAP(output_metric = "hyperboloid", n_components=latent_dim, n_neighbors = self.k, low_memory=True, init = "spectral")
                y = sparse.csr_matrix(reducer.fit(y[indices_sample]).transform(y))
                
            data_y, pointers_y, indices_y = y.data, y.indptr, y.indices
            weights = _compiled_multi_label_classification_update_weights(samples,
                                                                          self.num_iter,
                                                                          data,
                                                                          pointers,
                                                                          indices,
...
def evaluate_importances_with_logistic_regression(x_train, x_test, y_train, y_test, importances, k):
    sorted_args = np.argsort(importances)[::-1][0:k]
    clf_lr = LogisticRegression(solver="lbfgs", max_iter=100000).fit(x_train[:, sorted_args], y_train.ravel())
...
def experiment(x, y):
    ...
    #     n_samples = 100
    #     n_features = 500
    #     x, y = make_classification(n_samples=n_samples, n_features=n_features, n_informative=int(n_features / 5),
    #                                n_redundant=0, n_repeated=0, random_state=12, n_classes=4)

    # # a = np.random.randint(0,2,1000)
    # # b = np.random.randint(0,2,1000)
    # # d = np.random.rand(60, 1000)
    # # Y = np.abs(a-b)
    # # zgv = np.repeat(np.vstack([a,b]), 30, axis = 0)
    # # X = np.vstack([zgv, d]).T
    ...
    relief_b3_instance = ReliefE(embedding_based_distances=True, verbose=True, use_average_neighbour=True, determine_k_automatically=True, num_iter = wrange)
    ...
    # mat_obj = sio.loadmat("../preprocessed_sparse_matrices/p53.mat")
    # x = mat_obj['input_space']
    # y = mat_obj['target_space'].reshape(-1)  ## this is not one hot for scc
    # relief_b3_instance = ReliefE(embedding_based_distances=True, verbose=True, use_average_neighbour=True, determine_k_automatically=True)
    # relief_b3_instance.fit(x, y)
    # importances2 = relief_b3_instance.feature_importances_
    # for k,v in relief_b3_instance.timed.items():
    #     print(k+"\t"+str(v))
        ...
        # logging.info("Rebate ..")
        # clf4 = ReliefF(n_neighbors=neighbours).fit(np.asarray(x_train.todense(), dtype=np.float64), y_train)
        # importances1 = clf4.feature_importances_

        # if len(importances2.shape) == 2:
        #     importances2 = importances2[0, :]
        # assert len(importances1) == len(importances2), (len(importances1), len(importances2))
        # plt.plot(importances1, label="Rebata")
        # plt.plot(importances2, label="Belief")
        # plt.legend()
        # plt.show()

    # evaluate_importances = True

    # if evaluate_importances:  # joj kva je to grdo Ne vekej :) A je zdej bols? :) seveda!
    #     importances = [importances1, importances2]  # importances3
    #     names = ["Rebate", "ReliefF-this work", "ReliefE"][:-1]
    #     for enx, j in enumerate(importances):
    #         xax = []
    #         yax = []
    #         for k in tqdm.tqdm(np.arange(1, len(importances1), int(max(len(importances1), 100) / 100))):
    #             score = evaluate_importances_with_logistic_regression(x_train, x_test, y_train, y_test, j, k)
    #             xax.append(k)
    #             yax.append(score)
    #         sns.lineplot(xax, yax, label=names[enx])
    #     plt.legend()
    #     plt.xlabel("Top k")
    #     plt.ylabel("F1")
    #     plt.show()
...
def experiment2():
    ...
    print("ReliefF ..")
    relief_b3_instance = ReliefE(embedding_based_distances=False, verbose=True, k=neighbours)
    relief_b3_instance.fit(x_train, y_train)
    importances2 = relief_b3_instance.feature_importances_
    print(importances2)

    print("ReliefE ..")
    relief_b3_instance = ReliefE(embedding_based_distances=True, verbose=True, k=neighbours, latent_dimension=2)
    relief_b3_instance.fit(x_train, y_train)
    importances2 = relief_b3_instance.feature_importances_
    print(importances2)

    x_train, y_train, _ = load_arff(ova, features, targets[0])
    neighbours = 2
